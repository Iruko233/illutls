...
- **TLS 扩展随机化 (Extension Shuffling)**：默认开启，在建立连接时随机打乱 TLS 扩展顺序。此功能可模拟真实浏览器的扩展乱序机制，在生成动态 JA3 指纹的同时保持 JA4 哈希稳定。如有特殊需求，可通过 `WithShuffleExtensions(false)` 关闭。
- **ECH 扩展兼容**：通过为 ECH 扩展 (65037) 注入符合规范的 GREASE 占位数据，修复了因负载为空导致的部分 WAF（如 Cloudflare）握手解析失败问题，同时确保指纹特征不丢失。
