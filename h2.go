...
func ApplyH2Settings(t *http2.Transport, s H2Settings, windowUpdate uint32, priority H2Priority) {
	...
	if s.MaxFrameSize > 0 {
		t.Settings[http2.SettingMaxFrameSize] = s.MaxFrameSize
	}
	...
	if len(s.SettingsOrder) > 0 {
		for _, id := range s.SettingsOrder {
			t.SettingsOrder = append(t.SettingsOrder, http2.SettingID(id))
		}
	} else {
		if s.HeaderTableSize > 0 {
		...
		if s.MaxFrameSize > 0 {
			t.SettingsOrder = append(t.SettingsOrder, http2.SettingMaxFrameSize)
		}
		if s.MaxHeaderListSize > 0 {
